...
export class MongoProductRepository implements ICompanyRepository {
  ...
  async findAll(
    ...
    return ProductModel.find({isDeleted: false, ...payload}).populate("images").exec();
  }
  async findOneProduct(
    payload: FilterQuery<IProductDocument>
  ): Promise<IProductDocument | null> {
    return ProductModel.findOne({isDeleted: false, ...payload}).populate("images").exec();
  }
